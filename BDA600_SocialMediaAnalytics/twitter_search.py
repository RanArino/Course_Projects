...
def tw_oauth(authfile):
    ...
    """
    # the modified version of codes
    #  retrieve auth data from yaml
    with open(authfile, 'r') as file:
        ak = yaml.safe_load(file)
    #  assign confidential info
    auth1 = tweepy.auth.OAuthHandler(ak['apiKey'], ak['apiKeySecret'])
    auth1.set_access_token(ak['accessToken'], ak['accessTokenSecret'])
    return tweepy.API(auth1)


def tw_search_json(query, cnt=5):
    authfile = 'BDA600_SocialMediaAnalytics/auth.yml'
